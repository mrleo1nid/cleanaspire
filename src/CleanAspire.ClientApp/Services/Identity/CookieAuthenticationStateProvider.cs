...
) : AuthenticationStateProvider, ISignInManagement
{
    ...
    public override async Task<AuthenticationState> GetAuthenticationStateAsync()
    {
        ...
        try
        {
            ...
            {
                profileResponse = await indexedDb.GetDataAsync<ProfileResponse>(
                    IndexedDbCache.DATABASENAME,
                    CACHEKEY_CREDENTIAL
                );
            }
    ...
    )
    {
        ...
        try
        {
            ...
            {
                // Offline login logic
                var storedToken = await indexedDb.GetDataAsync<string>(
                    IndexedDbCache.DATABASENAME,
                    request.Email!
                );
                if (storedToken == null)
                {
                    throw new InvalidOperationException(
                        "No offline data available for the provided email."
                    );
                }
            }
