...
public static class IdentityApiAdditionalEndpointsExtensions
{
        ...
        where TUser : class, new()
    {
        ...
        var routeGroup = endpoints
            .MapGroup("/account")
            .WithTags("Authentication", "Account Management");
        routeGroup
            .MapPost(
                "/logout",
        ...
        routeGroup
            .MapGet(
                "/profile",
                async Task<Results<Ok<ProfileResponse>, ValidationProblem, NotFound>> (
                    ...
                    )
                    {
                        return CreateValidationProblem(
                            IdentityResult.Failed(
                                userManager.ErrorDescriber.InvalidEmail(request.Email)
                            )
                        );
                    }
            ...
            .MapPost(
                ...
                ) =>
                {
                    ...
                    if (
                        ...
                        && !_emailAddressAttribute.IsValid(request.NewEmail)
                    )
                    {
                        return CreateValidationProblem(
                            IdentityResult.Failed(
                                userManager.ErrorDescriber.InvalidEmail(request.NewEmail)
                            )
                        );
                    }
                    var email = await userManager.GetEmailAsync(user);
            ...
            .MapDelete(
                ...
                ) =>
                {
                    ...
                    if (string.IsNullOrEmpty(request.Username) || userName != request.Username)
                    {
                        return CreateValidationProblem(
                            IdentityResult.Failed(
                                userManager.ErrorDescriber.InvalidUserName(request.Username)
                            )
                        );
                    }
        ...
        routeGroup
            ...
            .Add(endpointBuilder =>
            {
                ...
                confirmEmailEndpointName =
                    $"{nameof(MapIdentityApiAdditionalEndpoints)}-{finalPattern}";
        ...
        routeGroup
            .MapGet(
                "/google/loginUrl",
                ([FromQuery] string state, HttpContext context) =>
                {
                    ...
                    if (!redirectUri.StartsWith(state))
                    {
                        return Results.ValidationProblem(
                            new Dictionary<string, string[]>
                            {
                                {
                                    "state",
                                    new[] { "The state parameter does not match the redirect URI." }
                                },
                            }
                        );
                    }
        ...
        routeGroup
            .MapPost(
                ...
                ) =>
                {
                    ...
                    if (string.IsNullOrEmpty(baseRedirectUri))
                    {
                        return Results.ValidationProblem(
                            new Dictionary<string, string[]>
                            {
                                {
                                    "baseRedirectUri",
                                    new[] { "Client base URL is not configured." }
                                },
                            }
                        );
                    }
                    ...
                    var authorizationCodeExchangeRequest = await httpClientFactory
                        .CreateClient()
                        .PostAsync("https://oauth2.googleapis.com/token", idTokenRequestContent);

                    if (!authorizationCodeExchangeRequest.IsSuccessStatusCode)
                    {
                        var responseMessage =
                            await authorizationCodeExchangeRequest.Content.ReadAsStringAsync();
                        return Results.BadRequest(
                            $"Authorization code exchange failed. {responseMessage}"
                        );
                    }

                    var idTokenContent =
                        await authorizationCodeExchangeRequest.Content.ReadFromJsonAsync<GoogleTokenResponse>();
                    if (idTokenContent?.id_token is null)
                    {
                        return Results.BadRequest(
                            "id_token not found in the response from the identity provider"
                        );
                    }
                    try
                    {
                        ...
                        if (user is null)
                        {
                            ...
                            appUser.RefreshTokenExpiryTime = DateTime.UtcNow.AddSeconds(
                                idTokenContent.expires_in
                            );
                            ...
                            await userManager.AddLoginAsync(
                                user,
                                new UserLoginInfo("Google", validatedUser.Subject, "Google")
                            );
                        }
                    ...
                    {
                        return Results.BadRequest(
                            $"The id_token did not pass validation. {e.Message}"
                        );
                    }
                    catch (Exception e)
                    {
        ...
        routeGroup
            .MapPost(
                ...
                ) =>
                {
                    if (string.IsNullOrEmpty(code))
                    {
                        return Results.ValidationProblem(
                            new Dictionary<string, string[]>
                            {
                                {
                                    "code",
                                    new[] { "The authorization code parameter is required." }
                                },
                            }
                        );
                    }
                            ...
                            {
                                {
                                    ...
                                    new[]
                                    {
                                        "Microsoft Client ID or Client Secret is not configured.",
                                    }
                                },
                            }
                        );
                    ...
                    var tokenResponse = await httpClientFactory
                        .CreateClient()
                        .PostAsync(
                    ...
                    var tokenContent =
                        await tokenResponse.Content.ReadFromJsonAsync<MicrosoftTokenResponse>();
                    if (tokenContent?.access_token is null)
                    {
                        return Results.BadRequest(
                            "access_token not found in the response from the identity provider"
                        );
                    }
                    ...
                    try
                    {
                        ...
                        var oid =
                            ...
                            ?? tokenContent.id_token;
                        if (string.IsNullOrEmpty(oid))
                        {
                            return Results.BadRequest("OID not found in the token claims.");
                        }
                        var providerKey = oid!; // oid is guaranteed to be non-null after the check above
                        var given_name = principal
                        ...
                        if (user == null)
                        {
                            ...
                            {
                                throw new InvalidCastException(
                                    "Expected user to be ApplicationUser."
                                );
                            }
                            ...
                            await userManager.AddLoginAsync(
                                user,
                                new UserLoginInfo("Microsoft", providerKey, "Microsoft")
                            );
                        }
                        else
                        {
        ...
        routeGroup
            .MapGet(
                ...
                async Task<Results<Ok<AuthenticatorResponse>, ValidationProblem, NotFound>> (
                    ClaimsPrincipal claimsPrincipal,
                    HttpContext context,
        ...
        routeGroup
            .MapPost(
                "/enable2fa",
                async Task<Results<Ok, ValidationProblem, NotFound, BadRequest>> (
                    ClaimsPrincipal claimsPrincipal,
                    HttpContext context,
        ...
        routeGroup
            ...
            .WithDescription(
                "Generates new recovery codes if two-factor authentication is enabled. "
            ...
            .MapPost(
                ...
                ) =>
                {
                    ...
                    if (user is not null && await userManager.IsEmailConfirmedAsync(user))
                    {
                        ...
                        code = WebEncoders.Base64UrlEncode(Encoding.UTF8.GetBytes(code));
                        var resetLink =
                            $"{clientBaseUrl}/account/confirm-password-reset?email={email}&code={code}";
    ...
    private static async Task<ProfileResponse> CreateInfoResponseAsync<TUser>(
        TUser user,
        UserManager<TUser> userManager
    )
        where TUser : class
